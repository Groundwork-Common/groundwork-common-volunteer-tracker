 ...
 * file drives gwcvt_handle_save_event() through $_POST exactly as the browser
 * fills it, and after each save it asserts BOTH what is in the database AND what
...
update_option( GWCVT_SETTINGS_OPTION, array( 'shifts_enabled' => true, 'signup_enabled' => true, 'schedule_page' => 1 ) );
...
$gwcvt_landed = gwcvt_ed_save(
	...
	array(
		'0' => array(
			'name'  => 'Greeter',
			'slots' => array(
				'0' => gwcvt_ed_slot( $gwcvt_busy, $gwcvt_when, '09:00', '12:00', array( 'remove' => '1' ) ),
				'1' => gwcvt_ed_slot( (int) $gwcvt_slots[1], $gwcvt_when, '13:00', '15:00' ),
			),
		),
	),
	array( 'gwcvt_reason' => 'We have enough greeters', 'gwcvt_notify' => '1' )
);
...
gwcvt_ed_check( 'the notice says one was cancelled', '1' === (string) ( $gwcvt_landed['gwcvt_cancelled'] ?? '' ), (string) ( $gwcvt_landed['gwcvt_cancelled'] ?? '-' ) );
...
gwcvt_ed_check( 'it no longer offers to cancel it again', false === strpos( $gwcvt_row, '[remove]' ) );
gwcvt_ed_check( 'it offers to put it back on', false !== strpos( $gwcvt_row, '[restore]' ) );
...
$gwcvt_landed = gwcvt_ed_save(
	...
	array(
		'0' => array(
			'name'  => 'Greeter',
			'slots' => array(
				'0' => array( 'id' => (string) $gwcvt_busy, 'date' => $gwcvt_when, 'start' => '09:00', 'end' => '12:00', 'restore' => '1' ),
			),
		),
	)
);
...
gwcvt_ed_check( 'the notice says so', '1' === (string) ( $gwcvt_landed['gwcvt_restored'] ?? '' ), (string) ( $gwcvt_landed['gwcvt_restored'] ?? '-' ) );
...
$gwcvt_landed = gwcvt_ed_save(
	...
	array(
		'0' => array(
			'name'  => 'Set-up',
			'slots' => array( '0' => gwcvt_ed_slot( $gwcvt_empty, $gwcvt_when, '07:30', '09:00', array( 'remove' => '1' ) ) ),
		),
	)
);

gwcvt_ed_check( 'an empty time is deleted outright', null === get_post( $gwcvt_empty ) );
gwcvt_ed_check( 'the notice says deleted, not cancelled', '1' === (string) ( $gwcvt_landed['gwcvt_deleted'] ?? '' ) && ! isset( $gwcvt_landed['gwcvt_cancelled'] ) );
...
$gwcvt_landed = gwcvt_ed_save(
	...
	array(
		'0' => array(
			...
			'remove' => '1',
			'slots'  => array(
				'0' => gwcvt_ed_slot( (int) $gwcvt_kept[0], $gwcvt_when, '09:00', '12:00' ),
				'1' => gwcvt_ed_slot( (int) $gwcvt_kept[1], $gwcvt_when, '13:00', '15:00' ),
			),
		),
	),
	array( 'gwcvt_reason' => 'Not greeting this year', 'gwcvt_notify' => '1' )
);
