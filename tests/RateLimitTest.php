...
final class RateLimitTest extends TestCase {
	...
	private function attempts( int $times, string $ip = self::IP, string $email = self::EMAIL ): int {
		...
		for ( $i = 0; $i < $times; $i++ ) {
			if ( gwc_vt_rate_limited( $ip, $email ) ) {
				++$refused;
			}
		}
	...
	public function test_the_email_window_is_the_tightest(): void {
		...
		$this->assertTrue( gwc_vt_rate_limited( self::IP, self::EMAIL ) );
	}
	...
	public function test_one_address_cannot_be_worked_around_by_changing_email(): void {
		...
		for ( $i = 0; $i < 10; $i++ ) {
			gwc_vt_rate_limited( self::IP, 'person' . $i . '@example.test' );
		}

		$this->assertTrue(
			gwc_vt_rate_limited( self::IP, 'someone-new@example.test' ),
	...
	public function test_a_refused_attempt_still_counts(): void {
		...
		$before = get_option( GWC_VT_RATE_LIMIT_OPTION );
		gwc_vt_rate_limited( self::IP, self::EMAIL );
		$after = get_option( GWC_VT_RATE_LIMIT_OPTION );
	...
	public function test_a_different_address_is_a_different_window(): void {
		...
		$this->assertFalse(
			gwc_vt_rate_limited( '198.51.100.22', 'someone-else@example.test' ),
	...
	public function test_a_missing_ip_does_not_bypass_the_email_window(): void {
		...
		for ( $i = 0; $i < 6; $i++ ) {
			gwc_vt_rate_limited( '', self::EMAIL );
		}

		$this->assertTrue( gwc_vt_rate_limited( '', self::EMAIL ) );
	}
	...
	public function test_the_windows_are_filterable(): void {
		...
		add_filter( 'gwc_vt_rate_limits', $tighter );

		$this->assertFalse( gwc_vt_rate_limited( self::IP, self::EMAIL ) );
		$this->assertTrue( gwc_vt_rate_limited( self::IP, self::EMAIL ) );

		remove_filter( 'gwc_vt_rate_limits', $tighter );
	}

	public function test_the_email_key_is_case_insensitive(): void {
		for ( $i = 0; $i < 6; $i++ ) {
			gwc_vt_rate_limited( '', 'Jane@Example.Test' );
		}
	...
	public function test_addresses_are_not_stored_in_the_clear(): void {
		...
		$stored = wp_json_encode( get_option( GWC_VT_RATE_LIMIT_OPTION ) );
	...
	public function test_a_closed_window_is_pruned(): void {
		...
		update_option( GWC_VT_RATE_LIMIT_OPTION, $store );

		gwc_vt_rate_limited( '198.51.100.99', 'other@example.test' );
	...
	public function test_a_scope_that_somehow_explodes_is_emptied(): void {
		...
		for ( $i = 0; $i < GWC_VT_RATE_LIMIT_CEILING + 5; $i++ ) {
			$bloated[ 'key' . $i ] = array(
				'count' => 1,
				'start' => time(),
			);
		...
		update_option( GWC_VT_RATE_LIMIT_OPTION, array( 'ip' => $bloated ) );
		...
		$this->assertCount( 1, get_option( GWC_VT_RATE_LIMIT_OPTION )['ip'] );
	}
}
