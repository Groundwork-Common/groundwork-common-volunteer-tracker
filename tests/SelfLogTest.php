...
final class SelfLogTest extends TestCase {
	...
	public function test_the_form_is_off_by_default(): void {
		 ...
		 * installed. */
		$this->assertFalse( gwcvt_self_log_enabled() );
	}
	...
	public function test_enabled_and_pinned_is_on(): void {
		...
		$this->assertTrue( gwcvt_self_log_enabled() );
	}
	...
	public function test_the_form_renders_nothing_while_it_is_off(): void {
		...
		$this->assertSame( '', gwcvt_render_self_log_form() );
	}

	/* ── The timing stamp ────────────────────────────────────────────────── */

	public function test_a_fresh_stamp_reads_as_new(): void {
		$this->assertSame( 0, gwcvt_form_age( gwcvt_form_stamp() ) );
	}

	public function test_a_stamp_from_the_past_reads_its_age(): void {
		$then  = time() - 300;
		$stamp = $then . '.' . hash_hmac( 'sha256', (string) $then, wp_salt( 'gwcvt_form' ) );

		$this->assertSame( 300, gwcvt_form_age( $stamp ) );
	}
	...
	public function test_a_forged_stamp_is_refused(): void {
		...
		$this->assertNull( gwcvt_form_age( $then . '.deadbeef' ) );
		$this->assertNull( gwcvt_form_age( (string) $then ) );
		$this->assertNull( gwcvt_form_age( '' ) );
		$this->assertNull( gwcvt_form_age( 'not-a-stamp.at-all' ) );
	}
	...
	public function test_a_stamp_from_the_future_does_not_read_as_negative(): void {
		 ...
		 * bot. */
		$this->assertSame( 0, gwcvt_form_age( $stamp ) );
	}

	public function test_the_bounds_are_what_the_handler_checks(): void {
		$this->assertSame( 3, GWCVT_FORM_MIN_AGE );
		$this->assertSame( 6 * HOUR_IN_SECONDS, GWCVT_FORM_MAX_AGE );
	}
	...
	public function test_accepted_honeypotted_and_rate_limited_are_byte_identical(): void {
		$accepted = gwcvt_self_log_message( 'accepted' );
		...
		foreach ( array( 'accepted', 'accepted', 'accepted' ) as $path ) {
			$this->assertSame( $accepted, gwcvt_self_log_message( $path ) );
		}
	}
	...
	public function test_an_unknown_result_says_nothing_at_all(): void {
		...
		$this->assertSame( '', gwcvt_self_log_message( 'something-else' ) );
		$this->assertSame( '', gwcvt_self_log_message( '' ) );
	}
	...
	public function test_the_messages_never_mention_a_person(): void {
		foreach ( array( 'accepted', 'incomplete', 'bad-code', 'expired' ) as $result ) {
			$message = strtolower( gwcvt_self_log_message( $result ) );
	...
	public function test_the_wrong_code_is_told_apart_from_everything_else(): void {
		 ...
		 * unusable. */
		$this->assertNotSame( gwcvt_self_log_message( 'accepted' ), gwcvt_self_log_message( 'bad-code' ) );
	}
}
