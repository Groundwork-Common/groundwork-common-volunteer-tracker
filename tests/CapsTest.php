...
final class CapsTest extends TestCase {
	...
	public function test_the_three_gates_are_distinct(): void {
		$this->assertSame( 'gwcvt_verify_hours', gwcvt_cap( 'verify' ) );
		$this->assertSame( 'gwcvt_issue_letters', gwcvt_cap( 'issue' ) );
		$this->assertSame( 'manage_options', gwcvt_cap( 'manage' ) );
	...
	public function test_a_site_can_remap_the_capabilities(): void {
		...
		add_filter( 'gwcvt_capabilities', $remap );

		$this->assertSame( 'edit_shifts', gwcvt_cap( 'verify' ) );
		$this->assertSame( 'gwcvt_issue_letters', gwcvt_cap( 'issue' ), 'Remapping one must not disturb the others.' );

		remove_filter( 'gwcvt_capabilities', $remap );
	}

	public function test_a_filter_that_drops_a_key_fails_closed(): void {
		$broken = static fn(): array => array();

		add_filter( 'gwcvt_capabilities', $broken );
		...
		$this->assertSame( 'manage_options', gwcvt_cap( 'verify' ) );

		remove_filter( 'gwcvt_capabilities', $broken );
	}
	...
	public function test_it_grants_both_capabilities_to_the_default_roles(): void {
		$admin  = gwcvt_test_add_role( 'administrator', array( 'manage_options' => true ) );
		$editor = gwcvt_test_add_role( 'editor', array( 'edit_posts' => true ) );

		gwcvt_grant_capabilities();

		foreach ( array( $admin, $editor ) as $role ) {
			$this->assertTrue( $role->has_cap( 'gwcvt_verify_hours' ), $role->name );
			$this->assertTrue( $role->has_cap( 'gwcvt_issue_letters' ), $role->name );
		}
	}

	public function test_granting_twice_does_not_rewrite_the_role(): void {
		$admin = gwcvt_test_add_role( 'administrator' );
	...
	public function test_a_capability_explicitly_set_to_false_stays_revoked(): void {
		$editor = gwcvt_test_add_role( 'editor' );
		 ...
		 * and the isset() guard in gwcvt_grant_capabilities() leaves it alone. */
		$editor->capabilities['gwcvt_issue_letters'] = false;

		gwcvt_grant_capabilities();

		$this->assertFalse(
			$editor->has_cap( 'gwcvt_issue_letters' ),
	...
	public function test_a_capability_removed_entirely_is_treated_as_lost_and_restored(): void {
		...
		gwcvt_grant_capabilities();
		$editor->remove_cap( 'gwcvt_issue_letters' );
		 ...
		 * gwcvt_grant_capabilities(); asserted here so that changing it is a
		 * decision rather than an accident. */
		$this->assertTrue( $editor->has_cap( 'gwcvt_issue_letters' ) );
	}
	...
	public function test_a_missing_role_is_not_an_error(): void {
		...
		$this->assertTrue( true, 'gwcvt_grant_capabilities() must survive a site with no roles.' );
	}

	public function test_the_granted_roles_are_filterable(): void {
		$volunteers = gwcvt_test_add_role( 'volunteer_coordinator' );
		$only_ours  = static fn(): array => array( 'volunteer_coordinator' );

		add_filter( 'gwcvt_default_cap_roles', $only_ours );
		gwcvt_grant_capabilities();
		remove_filter( 'gwcvt_default_cap_roles', $only_ours );

		$this->assertTrue( $volunteers->has_cap( 'gwcvt_verify_hours' ) );
	}

	/* ── The verification check ──────────────────────────────────────────── */

	public function test_verifying_needs_both_the_capability_and_access_to_the_record(): void {
		gwcvt_test_add_user( 7, array( 'gwcvt_verify_hours' => true, 'edit_post' => true ) );

		$this->assertTrue( gwcvt_user_can_verify( 7, 42 ) );
	}

	public function test_the_capability_alone_is_not_enough(): void {
		gwcvt_test_add_user( 8, array( 'gwcvt_verify_hours' => true ) );

		$this->assertFalse(
			gwcvt_user_can_verify( 8, 42 ),
	...
	public function test_editing_posts_alone_is_not_enough(): void {
		gwcvt_test_add_user( 9, array( 'edit_posts' => true, 'edit_post' => true ) );

		$this->assertFalse(
			gwcvt_user_can_verify( 9, 42 ),
	...
	public function test_a_logged_out_visitor_can_verify_nothing(): void {
		$this->assertFalse( gwcvt_user_can_verify( 0, 42 ) );
		$this->assertFalse( gwcvt_user_can_verify( 7, 0 ) );
	}

	public function test_issuing_a_letter_is_gated_separately(): void {
		gwcvt_test_add_user( 10, array( 'gwcvt_verify_hours' => true, 'edit_post' => true ) );

		$this->assertTrue( gwcvt_user_can_verify( 10, 42 ) );
		$this->assertFalse(
			gwcvt_user_can_issue( 10 ),
