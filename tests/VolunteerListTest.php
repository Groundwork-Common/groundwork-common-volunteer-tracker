 ...
 * and until now that link was a bare edit.php?post_type=gwc_vt_volunteer: every
...
final class VolunteerListTest extends TestCase {
	...
	public function test_overdue_lists_exactly_the_ids_it_was_given(): void {
		$vars = gwc_vt_volunteer_query_vars( 'overdue', '', 'ASC', array( 7, 9, 12 ) );

		$this->assertSame( array( 7, 9, 12 ), $vars['post__in'] );
	}

	public function test_NOBODY_OVERDUE_LISTS_NOBODY(): void {
		$vars = gwc_vt_volunteer_query_vars( 'overdue', '', 'ASC', array() );
	...
	public function test_has_a_requirement_asks_for_a_positive_figure(): void {
		$vars = gwc_vt_volunteer_query_vars( 'has' );

		$this->assertSame( GWC_VT_VOLUNTEER_REQUIRED, $vars['meta_query'][0]['key'] );
	...
	public function test_no_requirement_covers_unset_and_zero(): void {
		$vars = gwc_vt_volunteer_query_vars( 'none' );
	...
	public function test_no_filter_asks_for_nothing(): void {
		$this->assertSame( array(), gwc_vt_volunteer_query_vars( '' ) );
	}

	public function test_an_unknown_state_is_ignored_rather_than_guessed(): void {
		$this->assertSame( array(), gwc_vt_volunteer_query_vars( 'made-up' ) );
	}

	/* ── The deadline sort ───────────────────────────────────────────────── */

	public function test_SORTING_BY_DEADLINE_KEEPS_VOLUNTEERS_WITHOUT_ONE(): void {
		$vars = gwc_vt_volunteer_query_vars( '', 'gwc_vt_required', 'ASC' );

		$this->assertSame( 'OR', $vars['meta_query']['relation'] );

		$compares = array(
			$vars['meta_query']['gwc_vt_deadline']['compare'],
	...
	public function test_the_sort_is_by_the_named_clause_not_the_bare_key(): void {
		$vars = gwc_vt_volunteer_query_vars( '', 'gwc_vt_required', 'DESC' );

		$this->assertSame( 'DESC', $vars['orderby']['gwc_vt_deadline'] );
	...
	public function test_a_bad_order_falls_back_to_ascending(): void {
		$vars = gwc_vt_volunteer_query_vars( '', 'gwc_vt_required', 'sideways' );

		$this->assertSame( 'ASC', $vars['orderby']['gwc_vt_deadline'] );
	}

	public function test_another_column_does_not_get_the_deadline_sort(): void {
		$vars = gwc_vt_volunteer_query_vars( '', 'title', 'ASC' );
	...
	public function test_the_sort_wins_over_a_filters_meta_query(): void {
		$vars = gwc_vt_volunteer_query_vars( 'has', 'gwc_vt_required', 'ASC' );

		$this->assertSame( 'OR', $vars['meta_query']['relation'] );
		$this->assertArrayHasKey( 'gwc_vt_deadline', $vars['meta_query'] );
	}

	/* ── The filter's own options ────────────────────────────────────────── */

	public function test_every_offered_state_does_something_except_the_default(): void {
		$options = gwc_vt_volunteer_filter_options();
		...
		foreach ( array_keys( $options ) as $state ) {
			...
			$this->assertNotSame(
				array(),
				gwc_vt_volunteer_query_vars( (string) $state, '', 'ASC', array( 1 ) ),
	...
	public function test_the_sortable_column_is_registered(): void {
		$this->assertArrayHasKey( 'gwc_vt_required', gwc_vt_volunteer_sortable_columns( array() ) );
	}
}
