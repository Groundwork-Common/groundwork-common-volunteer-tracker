...
- **There are two REST routes**, both read-only. `gwc-vt/v1/volunteers` serves the entry screen's volunteer picker. `gwc-vt/v1/recurrence-preview` answers "how many shifts would this make" on the add-a-shift form, and touches no record at all — it is `gwc_vt_recurrence_dates()` over three form values, which is the point: the alternative is a second copy of the recurrence rule in JavaScript, drifting from the one the save uses. Every post type stays `show_in_rest => false`. The siblings' "no REST" rule is really an argument against *auto-generated* CPT routes, which is right and is kept; for a purpose-built lookup, `permission_callback` and an `args` schema are better guard rails than a hand-rolled `check_ajax_referer` that fails open when forgotten.
